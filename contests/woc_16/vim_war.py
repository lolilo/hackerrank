...
def get_valid_armies(num_soldiers, soldiers_array, skillset_requirement, num_possible_skills):
    valid_army_count = 0
    for length in xrange(num_soldiers + 1):
        possible_armies = findsubsets(range(num_soldiers), length)
...
def hackerrank():
    ...
    soldiers_array = []
    for _ in xrange(num_soldiers):
        skill_to_soldier_array = [int(x) for x in list(raw_input())]
        soldiers_array.append(skill_to_soldier_array)
    skillset_requirement = [int(x) for x in list(raw_input())]
